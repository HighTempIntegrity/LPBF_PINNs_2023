...
def generator_points(samples, dim, random_seed, type_of_points, boundary):
    ...
    if type_of_points == "sobol":
        ...
        unimat = np.full(dim, 1.0)
        #  data = (data - unimat * np.amin(data, axis=0)) / ((np.amax(data, axis=0) - np.amin(data, axis=0)) * unimat)
        ...
        dataY = (dataY - np.nanmin(dataY)) / (np.nanmax(dataY) - np.nanmin(dataY)) 

        dataZ[:, 0] = np.random.beta(5, 1, samples)
        # dataZ = (dataZ - np.nanmin(dataZ)) / (np.nanmax(dataZ) - np.nanmin(dataZ)) 
        ...
        # dataX = (dataX - np.nanmin(dataX)) / (np.nanmax(dataX) - np.nanmin(dataX))
        ...
        for j in range(samples-n-1, samples): #Center-bias: some points directly in laser center
            ...
            theta = rnd[2] * np.pi / 2
            phi  = rnd[3] * 2 * np.pi
            #xspeed = (dataP[j,3] + 0.5) / xsize
            dataX[j, :] = x0  + (datat[j, :] - t0) / tlaser * xspeed + radi * np.cos(phi) * np.sin(theta) / xsize * 3
            dataY[j, :] = 0.5 + radi * np.sin(phi) * np.sin(theta) / 2
            dataZ[j, :] = 1 - radi * np.cos(theta) / (1 + ring_thick)
            if(dim>4):
                dataP[j,:]= rnd[4:]

        print('max P: ', np.max(dataP[:,3]), ' | ', 'min P: ', np.min(dataP[:,3]))


        #datat = (datat - np.nanmin(datat)) / (np.nanmax(datat) - np.nanmin(datat))
